	...
	public static void main(String[] args) {
		...
		try {
			...
			while(reader.hasNextLine()) {
				...
				if(s.indexOf("\"") != -1) {
					if (s.indexOf(",") == -1) oldFollowers.add(s.trim());
			...
			while(reader.hasNextLine()) {
				String s = reader.nextLine();
				if(s.indexOf("\"") != -1) {
					if(s.indexOf(",") == -1) currentFollowers.add(s.trim());
	...
	public static ArrayList<Object> notInOther(ArrayList<Object> one, ArrayList<Object> two) {
		...
		for(int outer = 0; outer < one.size(); outer++) {
			...
			for(int inner = 0; inner < two.size(); inner++) {
				if(two.get(inner).equals(one.get(outer))) found = true;
			}
			if(!found) answer.add(one.get(outer));
		}
		return answer;
	}		
